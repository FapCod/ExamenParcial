...
INSERT INTO empleado VALUES( '0003', 'Reyes', 'Ortiz', 'Claudia', 'Lima', 'Av. Aviación 3456 - San Borja', 'aortiz', 'linda' );
...
-------------------2
CREATE PROCEDURE PAF_consulta_telefono_de_cliente (
@chr_cliecodigo char (5)
)
as 
begin 
select vch_clietelefono from Cliente where chr_cliecodigo = @chr_cliecodigo
end 
------------------------------3
create procedure PFA_consulta_saldo_de_cuenta(
@cod_cliecodigo char(5) 
)
as
begin 
SELECT        dbo.Cliente.chr_cliecodigo, dbo.Cliente.vch_cliepaterno, dbo.Cliente.vch_cliematerno, dbo.Cliente.vch_clienombre, dbo.Cuenta.dec_cuensaldo
FROM            dbo.Cliente INNER JOIN
                         dbo.Cuenta ON dbo.Cliente.chr_cliecodigo = dbo.Cuenta.chr_cliecodigo
where dbo.Cliente.chr_cliecodigo = @cod_cliecodigo
end
 exec PFA_consulta_saldo_de_cuenta '00001'



---------------------4
create procedure PAF_consultar_clientes_por_distrito(
@vch_cliedireccion VARCHAR(50)
)
as 
begin 
select * from cliente where vch_cliedireccion = @vch_cliedireccion
end

exec PAF_consultar_clientes_por_distrito 'Lince'
